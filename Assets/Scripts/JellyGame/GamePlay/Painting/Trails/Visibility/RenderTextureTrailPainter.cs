...
namespace JellyGame.GamePlay.Painting.Trails.Visibility
{
    ...
    public class RenderTextureTrailPainter : MonoBehaviour, IMovementPainter
    {
        ...
        {
            /*Debug.Log($"[PaintTimeAtUV] uvCenter={uvCenter}, halfSize={halfSizeUV}, opacity={opacity}");
            Debug.Log($"[PaintTimeAtUV] isFill={isFill}");*/
            if (!surface.EnableTimeAging || timeBrushBlitMaterial == null)
            {
                Debug.Log($"[PaintTimeAtUV] SKIPPED! EnableTimeAging={surface.EnableTimeAging}, material={timeBrushBlitMaterial != null}");
                return;
            }
            ...
            if (timeRT == null)
            {
                Debug.Log("[PaintTimeAtUV] SKIPPED! timeRT is null");
                return;
            }

            float currentTime = surface.GetCurrentTime();
            Debug.Log($"[PaintTimeAtUV] isFill={isFill}, time={currentTime}");
            ...
            Debug.Log($"[PaintTimeAtUV] Material name: {timeBrushBlitMaterial.name}, shader: {timeBrushBlitMaterial.shader.name}");
            float isFillValue = isFill ? 1f : 0f;
            Debug.Log($"[PaintTimeAtUV] Setting _IsFill to {isFillValue}");
            timeBrushBlitMaterial.SetFloat("_IsFill", isFillValue);

            // Verify it was set
            float checkValue = timeBrushBlitMaterial.GetFloat("_IsFill");
            Debug.Log($"[PaintTimeAtUV] _IsFill after set: {checkValue}");
